...
VERSION = "cbp-v1.0-anti-proxy-1"

# nuke any proxy envs that could poison the OpenAI client
for _k in (
...
def sanitize_text(s: str) -> str: return safe_str(s).strip()
def sanitize_key(s: str) -> str: return sanitize_text(s).replace(" ", "")
...
def get_openai():
    ...
    if _client is None:
        ...
        _client = OpenAI(api_key=key)  # default transport, no proxies
        try:
            import openai as _o
            print(f"[openai] client initialized (lib={getattr(_o,'__version__','unknown')})")
        except Exception:
            print("[openai] client initialized")
    return _client

bucket_name = os.environ.get("OUTPUT_BUCKET","memory-books-output")
DEFAULT_PROMPT = ("Convert this photo into a professional adult coloring book page with clean continuous "
...
try:
    ...
    print(f"[init] GCS not available: {safe_str(e)} | VERSION={VERSION}")
    storage_client = None; bucket = None

def extract_direct_image_url(url: str) -> str:
    url = sanitize_text(url); lower = url.lower()
    if lower.endswith((".jpg",".jpeg",".png",".gif",".webp")): return url
...
def download_image(url: str) -> bytes:
    ...
    with requests.get(direct, headers=headers, timeout=REQUEST_TIMEOUT, stream=True) as r:
        ...
        if "text/html" in ct: raise ValueError("Got HTML instead of image")
        total, chunks = 0, []
        for chunk in r.iter_content(chunk_size=8192):
            if chunk:
                total += len(chunk)
                if total > MAX_IMAGE_BYTES: raise ValueError("Image exceeds max size (20MB)")
                chunks.append(chunk)
...
def call_openai_edit(image_bytes: bytes, prompt: str) -> bytes:
    try:
        ...
        if not clean_prompt or len(clean_prompt) < 10: clean_prompt = "Convert to line art coloring book page"
        clean_prompt = clean_prompt.encode("ascii","ignore").decode("ascii")

        print(f"[openai] prompt: {clean_prompt[:120]}")

        try:
            resp = get_openai().images.edits(model="gpt-image-1", image=buf, prompt=clean_prompt, size="1024x1024")
        except AttributeError:
            resp = get_openai().images.edit(model="gpt-image-1", image=buf, prompt=clean_prompt, size="1024x1024")
        ...
        try:
            ...
            try:
                resp = get_openai().images.edits(model="gpt-image-1", image=buf, prompt="line art coloring page", size="1024x1024")
            except AttributeError:
                resp = get_openai().images.edit(model="gpt-image-1", image=buf, prompt="line art coloring page", size="1024x1024")
...
def upload_to_gcs(order_id: str, idx: int, img_bytes: bytes) -> str:
    ...
    try: blob.patch()
    except Exception as e: print(f"[gcs] patch failed: {safe_str(e)}")
    try: blob.make_public()
    except Exception as e: print(f"[gcs] make_public failed (likely uniform/PAP): {safe_str(e)}")
    url = blob.public_url
    if isinstance(url, bytes): url = url.decode("utf-8","ignore")
    if not url or url.startswith("gs://"): url = f"https://storage.googleapis.com/{bucket.name}/{blob_name}"
    return url

@app.route("/version", methods=["GET"])
def version():
    return safe_json_response({"version": VERSION})
    ...
    try:
        ...
        for idx, url in enumerate(image_urls):
            try:
                ...
                results.append({
                    "status":"ok","index":idx,"source_url":url,
...
if __name__ == "__main__":
    print(f"[boot] starting app VERSION={VERSION}")
