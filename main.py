...
# EXPERIMENTAL PROMPT - TESTING TRACE APPROACH
DEFAULT_PROMPT = (
   ...
   "TRACE WHAT EXISTS - Draw outlines only where edges actually appear in the photo. Every line must correspond to a real edge, boundary, or object from the original image. "
   "ENVIRONMENT RULE: Outdoor scenes with grass, fences, and sky must remain outdoor. Indoor scenes must remain indoor. Do not change the setting. "
   ...
   "BACKGROUND EXTRACTION: Trace the actual edges visible in the background. Where you see fence posts, draw fence posts. Where you see grass meeting sky, draw that horizon line. Where you see trees, draw their outline. Do not invent patterns or geometric shapes that are not in the photo. "
   "FORBIDDEN: Never add decorative patterns, abstract shapes, or geometric designs not present in the original. Never leave backgrounds completely blank if elements exist there. "
   "TECHNICAL EXECUTION: This is edge detection and tracing, not creative interpretation. Only draw lines where edges exist in the photo. "
   "FINAL CHECK: Output must have same number of people, same setting (indoor/outdoor), and trace real edges from the photograph."
)
