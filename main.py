...
# -------- helpers that read env lazily ----------
def _get_bucket_name() -> str:
    name = os.environ.get("OUTPUT_BUCKET", "").strip()
    if not name:
...
def _get_openai_client() -> OpenAI:
    return OpenAI(api_key=_get_openai_key())
...
def _as_png_rgb(raw: bytes) -> bytes:
    # Pillow import here so /health doesn’t pay the import cost or crash if Pillow missing
...
def _gcs_save_and_sign(data: bytes, key: str) -> Tuple[str, str]:
    ...
    return f"gs://{bucket.name}/{key}", url
...
@app.get("/health")
def health():
    # Never crash here — just report that the container is up
...
def process():
    ...
    for i, url in enumerate(image_urls):
        try:
            ...
            outputs.append({"index": i, "source_url": url, "status": "download_error", "detail": str(e)})
        except Exception as e:
            outputs.append({"index": i, "source_url": url, "status": "processing_error", "detail": str(e)})

    return jsonify({"order_id": order_id, "count": len(outputs), "prompt_used": prompt, "results": outputs}), 200
