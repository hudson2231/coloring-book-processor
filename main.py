...
OUTPUT_BUCKET = "memory-books-output"  # <— your hardcoded bucket name
DEFAULT_PROMPT = (
...
def _download(url: str) -> bytes:
    """
    Fetch bytes from a URL with a real User-Agent, follow redirects, and
    fail fast if the server returns HTML instead of an image.
    (Minimal change from your prior version.)
    """
    headers = {
        "User-Agent": "MemoryBooksBot/1.0 (+https://example.com) contact: support@example.com",
        "Accept": "*/*",
    }
    ...
    # If server tells us it's an image, return bytes
    if ctype.startswith("image/"):
        return r.content

    # Quick HTML detection (many wrapper pages / login pages)
    # If it's clearly HTML, error out so the caller can report a clean message.
    head = r.content[:2048].lower()
    if "text/html" in ctype or b"<html" in head or b"<!doctype html" in head:
        raise ValueError(f"URL did not return an image (content-type={ctype})")

    # Some servers omit content-type; return bytes anyway
    return r.content
